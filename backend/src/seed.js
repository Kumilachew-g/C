...
async function seed() {
  try {
    ...
    const commissionerPassword =
      process.env.SEED_COMMISSIONER_PASSWORD || "CommPass123!";
    const commissionerHash = await bcrypt.hash(
    ...
    // Secretariat user
    const secretariatPassword = process.env.SEED_SECRETARIAT_PASSWORD || "";
    ...
    // Auditor user
    const auditorPassword =
      process.env.SEED_AUDITOR_PASSWORD || "AuditorPass123!";
